...
export default function Home() {
  ...
  const { candles, loading, error, lastUpdate, isConnected, refetch } =
    useBinanceData(5000, currentSymbol);
  ...
  const [telegramService, setTelegramService] =
    useState<TelegramService | null>(null);
  ...
  const createSignalHash = useCallback(
    (signal: TradingSignal): string => {
      if (!signal) return "";
      return `${signal.action}_${signal.strength}_${
        Math.floor(signal.confidence / 5) * 5
      }_${Math.floor(signal.probability / 5) * 5}`;
    },
    []
  );
  ...
  const handleTestMessage = useCallback(async () => {
    ...
    const testSignal: TradingSignal = {
      ...
      stop_loss:
        candles.length > 0 ? candles[candles.length - 1].close * 0.98 : 49000,
      take_profit:
  ...
  useEffect(() => {
    ...
    const isWeakButHighConfidence =
      currentSignal.strength === "WEAK" && currentSignal.confidence >= 70;
    const isActionable =
    ...
    if (telegramConfig.enabled) {
      const shouldSendStrong =
        ...
        cooldownPassed;
      const shouldSendModerate =
        isModerateSignal &&
        isActionable &&
        currentSignal.probability >= 45 &&
        currentSignal.confidence >= 65 &&
        cooldownPassed;
      const shouldSendWeakHigh =
        isWeakButHighConfidence &&
        isActionable &&
        meetsProbabilityThreshold &&
        cooldownPassed;
      const shouldSendSignalChange =
        signalChanged &&
        isActionable &&
      ...
      if (shouldSend) {
        ...
        telegramService
          .sendTradingAlert(enhancedSignal, currentPrice)
          .then((success) => {
            if (success) {
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-2">
            Initializing scalping analysis...
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-2">
            Need at least 30 candles for scalping signals.
          </p>
        </div>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">
              ⚡ Lụm lúa cùng Tiến Anh - Scalping Pro
            </h1>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="text-blue-400 font-semibold mb-2">
                Enhanced Scalping Notifications
              </h3>
              <div className="text-blue-100 text-sm leading-relaxed space-y-1">
                <p>
                  • 🔥 <strong>STRONG signals</strong>: Confidence ≥55%,
                  Probability ≥35%
                </p>
                <p>
                  • 📈 <strong>MODERATE signals</strong>: Confidence ≥65%,
                  Probability ≥45%
                </p>
                <p>
                  • ⚡ <strong>HIGH CONFIDENCE</strong>: Weak signals with 70%+
                  confidence
                </p>
                <p>
                  • 🔄 <strong>SIGNAL CHANGES</strong>: Quick alerts on
                  direction shifts (15s cooldown)
                </p>
                <p>
                  • ⏱️ <strong>Reduced cooldown</strong>: 45 seconds between
                  regular signals
                </p>
              </div>
            </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="text-yellow-400 font-semibold mb-2">
                Scalping Risk Warning
              </h3>
              <div className="text-yellow-100 text-sm leading-relaxed space-y-1">
                <p>
                  🎯 <strong>Scalping Strategy</strong>: Quick in, quick out -
                  set tight stops!
                </p>
                <p>
                  💰 <strong>Position Size</strong>: Never risk more than 1-2%
                  per trade
                </p>
                <p>
                  ⏰ <strong>Time Management</strong>: Best during high
                  liquidity hours
                </p>
                <p>
                  🚫 <strong>Tham Lam Warning</strong>: Lương của mày chỉ được
                  500 cành / ngày...
                </p>
              </div>
            </div>
